...
- **Model 1 — 边缘检测**：主基线 **Time-RCD**（ICML 2026，Apache-2.0，HF 官方
  checkpoint，零样本泛化 —— 无逐任务训练、无标签，直接对 (T,C) 窗口打分；
  模块矩阵实测见 `docs/research/time-rcd-module-results.md`；组合阶段最优
  combo-fusion03-med5（robust-z 融合 w=0.3 → median-k5）VUS-PR 0.5248，
  较 base（0.383）提升 +0.142）。本项目检测器为
  `StreamingRobustDetector`（median/MAD + EWMA/POT 下界）、`FITSDetector`
  （频域 rFFT/低通/复线性插值重建）、`EdgeCascadeDetector`（事件环 → 不确定性
  门控 → FITS 频域趋势环 → 跨尺度一致性门控 → 状态条件分支，本项目创新）。
...
- **对照基线**：Time-RCD（主零样本基线）、TORAI（faithful 复现）、rcd_only
  （CausalRanker 消融）、BARO（median/IQR 单源）、correlation（Pearson 下界）、
  Anomaly Transformer（真实运行）。
